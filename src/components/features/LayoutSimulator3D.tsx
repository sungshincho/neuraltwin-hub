      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="font-semibold">매장 레이아웃 컨트롤</h4>
          ...
          <Card
            className="glass relative h-80 overflow-hidden"
            onDrop={handleDrop}
          ...
          >
            {products.map((product) => (
              <div
                ...
                className={`absolute w-12 h-12 rounded-lg ${product.color} cursor-move flex items-center justify-center text-white font-bold shadow-lg transition-smooth hover:scale-110`}
          ...
          {isAiLayout && (
            <Badge className="w-full justify-center py-2">
              ...
              AI 최적화 레이아웃 적용됨 - 예상 매출 +{((aiMetrics.conversion - metrics.conversion) / metrics.conversion * 100).toFixed(1)}%
            </Badge>
          )}
        </div>
        ...
        {/* 성과 비교 차트 */}
        <div className="space-y-4">
          <h4 className="font-semibold">전환율 예측 비교</h4>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Card className="glass p-4">
              ...
              <div className="text-2xl font-bold gradient-text">{metrics.conversion.toFixed(1)}%</div>
            </Card>
            <Card className="glass p-4">
              <div className="text-sm text-muted-foreground">방문자</div>
              <div className="text-2xl font-bold gradient-text">{metrics.traffic}</div>
            </Card>
          </div>
        </div>
      </div>
