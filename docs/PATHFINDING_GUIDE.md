...
이 시스템은 **38개**의 사전 정의된 고객 동선 케이스를 사용합니다. 시간대에 따라 가중치를 적용하여 랜덤하게 케이스가 선택됩니다.
...
| Case | 유형 | 설명 |
|------|------|------|
| 31 | 신규 | (-,-) 사분면 깊이 탐색 후 체크아웃 |
| 32 | 재방문 | (-,-) 사분면만 방문 후 복귀 |
| 33 | 신규 | 왼쪽벽 따라 (-,-) 깊숙이 후 체크아웃 |
| 34 | 재방문 | (-,-) 짧은 탐색 |

### (+,+) 사분면 집중 - Case 35-38
앞쪽 오른쪽 영역 (x > 0, z > 0) 탐색:

| Case | 유형 | 설명 |
|------|------|------|
| 35 | 신규 | (+,+) 사분면 집중 탐색 후 체크아웃 |
| 36 | 재방문 | (+,+) 사분면만 둘러보기 |
| 37 | 신규 | (+,+) 철저히 탐색 후 (-,-) 거쳐 체크아웃 |
| 38 | 재방문 | (+,+) 입구 바로 옆 구역만 짧게 |
